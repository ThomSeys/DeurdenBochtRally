# 🏍️ BOCHTENKONING RALLY 2026 - EVENT EVALUATIE

**VZW Deur Den Bocht**  
*Evaluatie: 6 februari 2026*

---

## 📅 EVENT DATA

### Officiële Event Details

**Basis Informatie:**

- **Event Naam:** Bochtenkoning Rally 2026
- **Datum:** Zaterdag 8 augustus 2026
- **Editie:** 2026 (werkdocument)

---

## 🕐 OFFICIEEL DAGPROGRAMMA

### Zaterdag 8 Augustus 2026

#### **06:30 - Check-in & Welkom**

*"De dag start rustig. Haal je startpakket op - compleet met routeboek, goodiebag en je persoonlijke rallynummer. Voel de spanning stijgen terwijl motoren binnenkomen en de parking volloopt met gelijkgestemde rijders. Dit is het moment om die eerste connecties te maken, bikes te checken en je mentaal klaar te maken voor de rit die komt."*

#### **06:30-07:15 - Ontbijt & Koffie**

*"Vers gebrouwen koffie, warme croissants en die unieke pre-ride energie. Het ontbijtbuffet is meer dan voeding - het is waar de verhalen beginnen. Motorrijders verzamelen, plannen worden gedeeld, routes vergeleken. Sommigen gaan voor een quick espresso shot, anderen nemen de tijd voor een volledig ontbijt. De keuze is aan jou, maar de vibe is universeel: anticipatie gemengd met koffiegeur."*

#### **07:15 - Briefing: Kort & Krachtig**

*"Geen lange speeches. We weten dat je wilt rijden. In exact 10 minuten krijg je de essentiële info: rally zones, noodhulp protocol, best practices voor groepsritten, en wat te doen als je een mooie omweg vindt (spoiler: volg je gevoel). We coveren veiligheid zonder te preken, want respect voor de weg zit al in je DNA als je hier staat. Daarna: motoren starten."*

#### **07:20 - Rally Start - Het Begint**

*"07:20 uur. Motoren warmlopen, laatste gear checks, helm op. Het geluid van tientallen bikes die tot leven komen vult de lucht. De spanning is tastbaar. Geen startpistool, geen countdown - gewoon rollen wanneer jij klaar bent. Sommigen schieten direct weg, anderen nemen het relaxed. Er is geen 'te laat', alleen jouw moment. **Het organisatieteam? Ook op de motor. We rijden mee.** Dit is geen evenement dat we organiseren, het is een dag die we samen beleven."*

#### **11:30-14:30 - Lunch in de Ardennen**

*"Rond de middag (wanneer jij uitkomt) stoppen we voor lunch op een van de mooiste plekken in de Ardennen. Een warme maaltijd, koude drank, en die typische post-ochtendrit vibe waar iedereen z'n verhaal heeft. Die ene hairpin. Dat onverwachte stuk asfalt. Die rijder die je tegenkwam. Hier komen routes samen en nieuwe plannen ontstaan voor de middag. Voor All-in deelnemers is dit included, anderen kunnen aanschuiven (reservering aanbevolen)."*

#### **17:00 - Finish @ Baraque de Fraiture**

*"Het hoogste punt van België. 652 meter boven zeeniveau, maar het voelt hoger na een dag als deze. De finish is geen streep maar een plek - met uitzicht, space om te chillen, en die typische einde-van-de-rit energie. Bikes parkeren, helmen af, eerste biertje open (of koffie, we oordelen niet). Hier komen alle verhalen samen. Mensen die solo reden vinden elkaar. Groepen die de hele dag samen optrokken reflecteren. De motor koelt af, maar de herinneringen zijn vers."*

#### **17:00-20:30 - Diner & Debrief**

*"Een uitgebreide maaltijd met iedereen die de dag gereden heeft. Dit is waar kilometers transformeren naar verhalen. De tafel vult zich met anekdotes, gelach en die herkenning van gedeelde ervaringen. Sommige verhalen gaan over techniek - die perfecte apex, dat moment van flow. Andere over ontmoetingen, verrassingen, close calls. Het eten is goed, maar de compagnie maakt het. En ja, er is genoeg voor iedereen - inclusief vegetarische opties en alternatieven voor specifieke diëten. Wie in elke rally zone minstens 1 challenge heeft afgerond, krijgt gratis drinken in de avond."*

#### **21:00 - Community Avond & Afsluiter**

*"De dag sluit af, maar niemand wil naar huis. We delen de beste foto's van de dag op het grote scherm - die ene corner die iedereen raakte, dat spontane groepsshot, de technische details die riders vastlegden. Sommigen delen hun volledige verhaal, anderen hangen gewoon rond met een drankje. Er zijn geen speeches, geen awards ceremony (we doen niet aan winnaars). Gewoon mensen die een passie delen en een dag samen beleefden. Dit is waar volgende edities geboren worden, waar nieuwe vriendschappen ontstaan, en waar je beseft: dit is waarom we rijden."*

---

## 📊 HOE HET EVENT WERKT

### Voor Deelnemers - De Volledige Ervaring

**🏁 Vooraf - Registratie**

Deelnemers registreren online en kiezen hun formule:

- **€30 All-in:** Ontbijt, lunch én diner inbegrepen
- **€20 Ontbijt only:** Alleen ontbijt, lunch & diner zelf regelen

Na betaling krijgen ze automatisch:

- Persoonlijke QR code (via email)
- Link naar PDF roadbook met routes
- Toegang tot hun persoonlijk dashboard
- Optie: papieren roadbook aanvragen

**🎯 Kies Je Eigen Avontuur**

Deelnemers hebben 2 opties:

**Optie 1 - Adventure Track (aanbevolen):**
Het event heeft geen volledig vaste route - ontdek 4 rally zones in je eigen tempo:

1. **Vlaamse Ardennen** - Heuvels met panoramische uitzichten
2. **Condroz** - Historische rally omgeving
3. **Ardennen - Ourthe Vallei** - Prachtige riviervalleien
4. **Hoge Venen** - Bossen en natuurgebieden

Per zone krijgen deelnemers meerdere route tips (panoramisch, technisch, vlot) zodat iedereen zijn eigen avontuur kan samenstellen.

**Met deze optie krijg je:**

- Check-ins per zone
- Route challenges voor extra punten
- Achievements & badges
- Leaderboard ranking (optioneel)
- Buddy tracking
- Maximale vrijheid & gamification

**Optie 2 - Classic Route (volg de GPX):**
Liever gewoon één mooie route volgen? Geen probleem! Download de volledige GPX track en rijd van ochtend tot avond.

**Met deze optie krijg je:**

- Één complete GPX route (klaar om te volgen)
- Garantie van een prachtige rit door alle mooiste plekken
- Minder interactie/gamification
- Wel toegang tot foto's delen & ride stories

Je mist de challenges, achievements en zone check-ins, maar je hebt wel een fantastische dag op de motor voor de boeg!

**📱 Het Dashboard - Alles in Één App**

Deelnemers hebben toegang tot een persoonlijk dashboard waar ze:

- **Adventure Track:** Zones inchecken met QR code, voortgang bijhouden, challenges doen
- **Beide opties:** Foto's delen, andere deelnemers taggen, ride stories schrijven
- **Beide opties:** Buddies toevoegen om samen te rijden
- **Adventure Track:** Achievements verzamelen (badges), live leaderboard bekijken
- **Beide opties:** GPX routes downloaden

Kies je voor de Adventure Track? Dan heb je toegang tot het volledige gamification systeem. Kies je voor de Classic Route? Dan focus je op de rit en houdt het dashboard simpel.

Alles werkt als Progressive Web App!
**📱 App Installatie - Direct op Je Smartphone** 🚀

Op het dashboard zie je automatisch een banner om de app te installeren op je smartphone - 1-klik en klaar! Dit geeft je:

- **Snellere toegang:** De app opent direct zonder browser
- **Offline support:** Werkt ook zonder internet (bepaalde functionaliteiten)
- **Betere performance:** De app draait sneller dan via browser
- **Notificaties:** Krijgt meldingen direct op je telefoon voor live updates

Eenmaal weggeklikt, verschijnt de banner niet meer automatisch. Maar je kunt deze altijd opnieuw activeren via een knop in je instellingen - dus je bent nooit vastgeketend.
**� Rondleiding - Stap voor Stap Uitleg** 🤖

Nieuwe deelnemers krijgen een volledige interactieve rondleiding door de app! De tour gaat over **meerdere pagina's heen** en toont precies waar je op moet klikken:

**Master Tour (volledige rondleiding - 21 stappen):**

- Start op het dashboard met welkomstbericht
- Toont PWA installatie (1-klik installeren op smartphone!) 🚀
- Toont meldingen (push notifications activeren voor live updates)
- Laat je profiel, QR code, en rally zone overzicht zien
- Navigeert naar Rally pagina en toont segmenten & route tips
- Gaat naar Naftgenoten (Riding Buddies) pagina voor groepsrijen
- Eindigt op Live Kaart voor real-time zone tracking & event markers
- Toont fotogalerij, ride stories, achievements, profiel, noodcontacten, checklist
- Terug naar dashboard voor afsluitend bericht

**Extra features in tour:**

- **PWA Install:** 1-klik installeren op je smartphone. Kan je altijd later weer activeren via een knop (eenmaal weggeklikt, verschijnt het niet automatisch meer)
- **Meldingen:** Krijg notificaties over zones, buddies en events. Kan je ook later inschakelen

**Pagina-specifieke tours:**

- Ben je al op Rally/Buddies/Live Map? Klik "Rondleiding" voor een korte tour alleen op die pagina

**Hoe het werkt:**

- De tour verschijnt automatisch voor nieuwe deelnemers
- Klik op "Volgende" om naar de volgende stap te gaan (inclusief navigatie naar andere pagina's)
- Klik "Vorige" om terug te gaan
- Via het help-icoon in de header kan je de volledige tour opnieuw starten

**�🎮 Route Challenges - Extra Uitdaging**

Bij specifieke locaties kunnen deelnemers challenges doen:

- **Foto opdrachten:** "Maak een foto van dit uitzicht"
- **Trivia vragen:** Multiple choice over de route/regio
- **Lokale kennis:** "Welk monument zie je hier?"

Multiple choice wordt instant gevalideerd, foto's worden later door ons team beoordeeld.

**Beloning:** Wie in **elke zone** minimaal 1 challenge afrondt, krijgt gratis drinken tijdens de avond!

**👥 Samen Rijden - Buddy Systeem**

Deelnemers kunnen "Naftgenoten" toevoegen:

- Zie wanneer je buddies inchecken bij zones
- Get notificaties: "Je buddy is aangekomen in Condroz!"
- Unlock groep achievements (badges voor samen rijden)
- Deel foto's en tag je buddies

Perfect voor mensen die samen rijden maar af en toe uit het oog verliezen.

**📸 Photo Albums - Herinneringen Delen**

Elk van de 4 zones heeft een eigen foto album:

- Upload max 5 foto's per zone
- Tag mede-rijders in je foto's (zij krijgen notificatie)
- Like foto's van anderen
- Beste foto's komen in featured gallery

Wij openen albums zone per zone tijdens het event, zodat niet alle foto's tegelijk binnen stromen.

**🏆 Achievements & Gamification**

Het systeem kent 10+ badges die automatisch unlocked:

- **First Check-in:** Eerste zone bereikt
- **Early Bird:** Voor 9u vertrokken
- **Zone Master:** Alle 4 zones afgerond
- **Photographer:** 10 foto's geüpload
- **Storyteller:** Ride story geschreven
- **Social Butterfly:** 5+ buddies toegevoegd
- En meer...

Punten verdienen door zones, challenges, foto's en stories. Leaderboard is optioneel (deelnemers kiezen zelf of ze zichtbaar zijn).

**🚨 Safety First**

Als er iets mis gaat:

- **SOS knop** in de app (stuurt direct GPS locatie naar ons)
- Noodcontacten kunnen vooraf ingesteld worden
- Locatie delen optioneel (buddies/familie kunnen meekijken)
- Noodtelefoon altijd bereikbaar

**📖 Na Het Event**

Deelnemers krijgen:

- Persoonlijk achievement certificaat
- Overzicht van hun stats (zones, afstand, achievements)
- Toegang tot alle foto's en stories
- Herinneringen aan hun rally ervaring

---

### Voor Medewerkers - Wat Moeten wij Doen?

**🛠️ Voorbereiding (Weken Voor Event)**

*Content Beheren (via gebruiksvriendelijke CMS):*

- Rally zones updaten (beschrijvingen, routes)
- Route challenges opstellen (foto opdrachten, trivia)
- Route tips toevoegen per zone (off-road, technisch, panoramisch)
- FAQ, schedule en homepage content up-to-date houden
- Event checklist doorlopen

*Geen code nodig - alles via visuele editor!*

**📋 Ochtend - Check-in (06:30-08:00)**

2-3 teamleden aan check-in tafel:

- QR codes scannen met tablet/telefoon (app doet de rest)
- Roadbooks & goodiebags uitdelen
- Vragen beantwoorden
- Briefing geven (07:30 - 10 minuten)

**🏍️ Tijdens Rally (08:00-17:00) - Team Rijdt Mee!**

Teamleden verdelen rollen tijdens het rijden:

*Nood Coördinator:*

- Noodtelefoon bereikbaar (handsfree)
- Dashboard checken bij stops (SOS meldingen?)
- Hulp coördineren indien nodig

*Route Shepherd:*

- Rijdt voorop, verkent route
- Checkt of zones bereikbaar zijn
- Waarschuwt team bij problemen

*Group Support:*

- Rijdt flexibel tussen deelnemers
- Helpt waar nodig

*Content Maker:*

- Foto's & video's voor social media
- Verhalen verzamelen

**👀 Moderatie - Wanneer Tijd Is (Lunch/Avond)**

Kort beoordelen in dashboard:

- **Foto challenges:** Goedkeuren/afwijzen met één click
- **Foto uploads:** Review en select featured content
- **Stories:** Check indien nodig (optioneel)
- **Photo albums:** Open/sluit per zone

Multiple choice challenges valideren zichzelf automatisch!

**📊 Admin Dashboard - Alles in Overzicht**

Dashboard toont realtime:

- SOS alerts (rood als actief)
- Hoeveel mensen ingecheckt
- Welke zones open/gesloten zijn
- Pending items (badges met cijfers)
- Check-in patronen
- Financiële status

Click door om acties te nemen - simpel en snel.

**🤖 Wat Doet Het Systeem Zelf?**

Automatisch zonder jullie:

- Registraties & betalingen
- QR codes versturen
- Emails sturen
- Check-ins registreren
- Achievements unlocken
- Buddy notificaties ("Je buddy is aangekomen!")
- Punten berekenen
- Leaderboard updaten
- Multiple choice valideren
- Certificaten genereren

**Jullie focussen op:**

- Check-in ochtend (belangrijk!)
- Rijden & genieten
- SOS monitoring (safety first)
- Foto/challenge review (als tijd is)

---

## 👥 TEAM EVALUATIE: 5 PERSONEN

### 🚨 BELANGRIJKSTE INZICHT

**Het organisatieteam rijdt mee!** Dit verandert ALLES:

✅ **Voordelen:**

- Team beleeft het event zelf (authenticiteit)
- Natuurlijke verspreiding over de route
- Real-time problemen kunnen spotten onderweg
- Community gevoel ("we rijden samen")
- Minder statisch wachten bij start/finish
- Kan deelnemers onderweg helpen

⚠️ **Consequenties:**

- Geen dedicated check-in team tussen 08:00-17:00
- Noodcoördinator moet bereikbaar blijven tijdens rijden
- Tech moet 100% autonoom werken
- Minder controle over situaties
- Fysiek veeleisender (hele dag rijden!)

---

## 📋 AANGEPASTE ROLVERDELING

### FASE 1: Voorbereiding (2-3 maanden voor event)

**Persoon 1 - Event Lead, Tech + Content & Marketing**

- Algemene coördinatie & tijdlijn
- Locaties boeken (start/eindpunt + lunch)
- Verzekeringen & legaal regelen
- Sponsoring contacten
- Tech stack monitoren & testen
- Sanity CMS management
- Social media content (15-20 posts)
- Event poster & promo materiaal
- Deelnemer communicatie (emails)
- FAQ's & website updates
- Budget beheer
- **Kritisch:** App moet 100% stabiel zijn (geen support tijdens rijden!)

**Persoon 2 - Route Master + Logistiek & Materiaal**

- 4 Rally zones uitwerken (routes, GPS)
- Route tips maken (meerdere per zone)
- Challenges opstellen in Sanity
- **MINIMUM 2 proefritten doen**
- Roadbook complete content
- Veiligheidscheck per route
- Noodprotocol opstellen
- Roadbooks design naar print
- 50-100 roadbooks laten printen
- Deelnemer goodiebags voorbereiden
- Signalisatie & spandoeken bestellen
- Transport naar locaties

**Persoon 3 - Operations & Finance**

- Betalingen monitoren (Stripe dashboard)
- Deelnemerslijst bijhouden - werkt al via app
- Check-in materialen voorbereiden - zijnde smartphone, zou afgehandeld moeten worden via app
- Email flows controleren
- Last-minute registraties - zijn uitgesloten wegens afsluiten van inschrijvingen 5 dagen op voorhand
- Budget tracking

---

### FASE 2: Evenementdag - 8 Augustus 2026

#### **OCHTEND (06:00-08:00) - ALLE 5/6 PERSONEN**

**06:00 - Setup (Team arriveert)**

- Persoon 1: Algemene coördinatie, laatste tech checks
- Persoon 2 & 3: Check-in tafel opzetten, tablets testen
- Persoon 4: Signalisatie, spandoeken, parking aanwijzen
- Persoon 5: Ontbijt coördinatie, goodiebags klaarzetten

**06:30-08:00 - Check-in & Ontbijt**

- **Persoon 2 & 3:** Check-in bemanning
  - QR codes scannen via smartphone
  - Roadbooks + goodiebags uitdelen
  - Korte uitleg geven per deelnemer
  - Vragen beantwoorden
- **Persoon 1:** Overzicht, tech support indien nodig
- **Persoon 4:** Parking organisatie, laatste materiaal checks
- **Persoon 5:** Foto's maken, social media, sfeer vastleggen

**07:30 - Briefing (10 minuten)**

- **Persoon 1 of 2** geeft briefing:
  - Rally zones uitleg
  - Noodprotocol (belangrijk!)
  - App/dashboard quick tour
  - Safety tips
  - Lunch info
  - "Ride safe, have fun!"

---

#### **08:00-17:00 - RALLY (TEAM RIJDT MEE!)**

**🏍️ 2 - 4 teamleden op de motor:**

- Rollen worden gecombineerd afhankelijk van wie meerijdt (minimum 2, ideaal 3-4)
- Bij 2 rijders: Nood Coordinator + Route Shepherd
- Bij 3 rijders: + Middle Support
- Bij 4 rijders: + Sweeper of Content (afhankelijk van focus)

**Persoon 1 - Nood Coordinator (KRITISCH)**

- ✅ Noodtelefoon bereikbaar (handsfree, Bluetooth)
- ✅ Monitort dashboard voor SOS meldingen (bij stops)
- ✅ Eerste aanspreekpunt bij problemen
- ✅ Coördineert hulp indien nodig
- ⚠️ Moet beschikbaar blijven HELE dag
- **Route:** Flexibel, blijft in bereik van meeste deelnemers

**Persoon 2 - Route Shepherd (Vooruit)**

- ✅ Rijdt voorop, verkent route
- ✅ Checkt of zones bereikbaar zijn
- ✅ Spot problemen (wegwerkzaamheden, gevaar, etc.)
- ✅ Update team bij issues
- **Route:** Volledige route, alle 4 zones

**Persoon 3 - Middle Support**

- ✅ Rijdt zelfstandig en blijft flexibel inzetbaar
- ✅ Helpt deelnemers bij vragen onderweg
- ✅ Community building (praat met mensen)
- ✅ Opvanger voor Nood Coordinator
- **Route:** Flexibel, bij voorkeur in de buurt van de meerderheid maar niet gebonden aan een groep

**Persoon 4 - Sweeper (Achterwacht)**

- ✅ Vertrekt als laatste om te checken of iedereen weg is - kan evengoed iemand zijn die niet meer vertrekt op de rit
- ✅ Helpt bij pech (of regelt hulp)
- ✅ Waarschuwt team bij vertraging en afstand tussen rijders
- ✅ Checkt regelmatig hoe ver iedereen zit
- **Route:** Flexibel, tempo vrij, volgt updates van de voorste rijders

**Persoon 5 - Content Creator**

- ✅ Maakt foto's & video's onderweg
- ✅ Plant strategische stops op mooie punten en blijft daar even voor shots
- ✅ Neemt onderweg video (on-bike of korte stops)
- ✅ Verzamelt verhalen van deelnemers
- ✅ Content creator is gedeelde taak: meerdere teamleden nemen content op
- **Route:** Flexibel, wisselt rijden af met korte stops

**🔗 Communicatie onderweg:**

- WhatsApp groep (checks bij stops)
- Noodtelefoon altijd bereikbaar
- Dashboard monitoring bij pauzes
- Update elkaar bij lunch

---

#### **11:30-14:30 - Lunch in Ardennen**

**Aanwezig: rijdende teamleden (2-4):**

- ✅ Check-in met deelnemers
- ✅ Verhalen verzamelen
- ✅ Groepsfoto's maken
- ✅ Social media update
- ✅ Dashboard checken (wie nog onderweg?)
- ✅ Plan middagrit communiceren
- ✅ Problemen oplossen indien nodig

**Korte teamoverleg (10 min):**

- Hoe gaat het tot nu toe?
- Technische issues?
- Deelnemer feedback?
- Plan voor middag

---

#### **14:30-17:00 - Namiddag Rally (verder rijden)**

**Zelfde verdeling als ochtend (rollen gecombineerd bij 2-4 rijders):**

- Persoon 1: Nood Coordinator
- Persoon 2: Vooruit
- Persoon 3: Middle Support  
- Persoon 4: Sweeper
- Persoon 5: Content

**Extra aandacht:**

- Moeheid bij deelnemers (lange dag)
- Weersverandering checken
- Laatste zones afronden
- Richting finish (Baraque de Fraiture)

---

#### **17:00-21:00+ - Finish @ Baraque de Fraiture**

**17:00 - Aankomst (Team & Deelnemers druppelen binnen)**

- **Persoon 2 & 3:** "Informele check-out"
  - QR scan bij aankomst (tablet)
  - Check of iedereen veilig is
  - Bikes parkeren aanwijzen
- **Persoon 4:** Foto's maken, groepsfoto organiseren
- **Persoon 5:** Social media finale update
- **Persoon 1:** Overzicht, checkt of iedereen binnen is (dashboard)

**17:00-20:30 - Diner**

- Team zit mee aan tafel (niet apart!)
- Verhalen delen met deelnemers
- Community vibe
- Ontspannen na lange dag
- Informeel feedback verzamelen
- Gratis drinken voor wie in elke rally zone minimaal 1 route tip, alle challenge afrondt

**21:00 - Community Avond & Foto's**

- **Persoon 4 & 5:** Foto's van de dag delen
- **Persoon 3:** Verhalen faciliteren
- **Alle teamleden:** Meepraten, netwerken, genieten
- **Persoon 1:** Event officieel afsluiten (kort woord)

**~22:00-23:00 - Afbraak & Einde**

- Persoon 2, 3, 4: Materiaal inpakken
- Persoon 1, 5: Laatste deelnemers uitzwaaien
- Team: Check of locatie proper is
- Evaluatie (15 min): Wat ging goed/fout?

---

## ✅ WERKBAAR MET 5 PERSONEN?

### 🟢 JA - Met deze setup WEL

**Waarom dit werkt:**

1. **Team rijdt mee = Authentiek**
   - Geen "wij vs deelnemers" maar "we doen het samen"
   - Team ervaart event zelf
   - Natuurlijke support onderweg
   - Betere community vibe

2. **Verdeelde verantwoordelijkheid onderweg**
   - Vooruit (verkent)
   - Middle (support)
   - Achter (vangnet)
   - Nood (bereikbaar)
   - Content (documenteert)

3. **Systeem draait vanzelf**
   - Check-ins via app (deelnemers doen zelf)
   - Achievements unlock automatisch
   - Leaderboard update automatisch
   - Buddy notificaties automatisch
   - QR codes werken zonder jullie hulp

4. **Minder cruciale momenten**
   - Check-in ochtend: genoeg tijd (1.5u)
   - Onderweg: geen bemanning nodig
   - Finish: informeel, geen stress
   - Diner: zit mee aan tafel

---

## ⚠️ RISICO'S & MITIGATIE

### HOGE RISICO'S

**1. Systeem storing tijdens event**

- ❌ **Risico:** App/website werkt niet, geen ondersteuning mogelijk
- ✅ **Mitigatie:**
  - 2 weken voor event: Alles uitgebreid testen
  - 1 week voor event: Geen wijzigingen meer!
  - Backup: Paper lijst met alle deelnemers + zones
  - WhatsApp groep voor deelnemers (info delen)
  - Developer op afstand bereikbaar (indien nodig)

**2. Noodsituatie tijdens rijden**

- ❌ **Risico:** Ongeval, team is niet direct ter plaatse
- ✅ **Mitigatie:**
  - Nood Coordinator heeft handsfree systeem
  - Backup: 2e persoon ook noodtelefoon (Persoon 3)
  - Duidelijke escalatie: Team → Hulpdiensten
  - SOS knop stuurt GPS locatie (team ziet waar deelnemer is)
  - Let op: Noodsituaties zijn voor organisatie + coördinatie, niet voor pechhulp. We kunnen eventueel pechhulp als sponsor zoeken, maar dat is geen kerntaak. We helpen coördineren, niet technisch oplossen.

**3. Teamlid ziek op eventdag**

- ❌ **Risico:** Met 5 personen is er geen buffer
- ✅ **Mitigatie:**
  - Week voor event: Alle teamleden healthy blijven (prioriteit!)
  - Backup optie: 1-2 reservepersonen briefen (vrienden/familie?)
  - Bij 4 personen: Scrap Content Creator (niet kritisch)
  - Bij 3 personen: Serieus probleem, bijkomend persoon moet aangesproken worden

**4. Lange dag = Moeheid**

- ❌ **Risico:** 06:00-23:00 = 17 uur, intensief rijden
- ✅ **Mitigatie:**
  - Goede nachtrust voor event (kritisch!)
  - Energierepen/snacks voor team
  - Koffie/hydratatie tijdens lunch
  - Pauzes nemen onderweg (niet non-stop rijden)
  - Van tevoren fitte zijn (conditie!)

### MEDIUM RISICO'S

**5. Communicatie onderweg**

- ❌ **Risico:** Team verspreid, moeilijk om elkaar te bereiken
- ✅ **Mitigatie:**
  - WhatsApp groep (checks bij elke stop)
  - Vooraf afspraken: Check-in elke 2u
  - Dashboard gebruiken om posities te zien
  - Lunch = centrale sync punt

**6. Deelnemers problemen**

- ❌ **Risico:** Meerdere deelnemers tegelijk hulp nodig
- ✅ **Mitigatie:**
  - Duidelijke instructies in roadbook
  - Veel info in de app/op website
  - Deelnemers kunnen elkaar helpen (community!)
  - Niet voor elk probleem is team nodig

**7. Weer**

- ❌ **Risico:** Slecht weer = gevaarlijk + minder leuk
- ✅ **Mitigatie:**
  - Week voor event: weersverwachting volgen
  - Bij code oranje/rood: communiceer alternatieven
  - Route aangepassen bij extreem weer

### LAGE RISICO'S

**8. Check-in chaos (ochtend)**

- ❌ **Risico:** 50 mensen in 1.5u kunnen druk zijn
- ✅ **Mitigatie:**
  - 2 personen aan check-in tafel
  - QR scan via app (snel, 15-20 sec per persoon)
  - Goodiebags vooraf klaargelegd (alfabetisch of genummerd)
  - Briefing kort houden (10 min)
  - Met 50p = zeer ruim haalbaar (zelfs piek momenten OK)

**9. Finish te laat**

- ❌ **Risico:** Sommige riders komen later dan 20:30
- ✅ **Mitigatie:**
  - App werkt altijd (ook na 20:30)
  - Team blijft tot laatste rider binnen is of neemt contact op met de deelnemer ivm stand van zaken
  - Communiceer tijden duidelijk

---

## 💡 SUCCESFACTOREN

### KRITISCH ✅

1. **Systeem moet betrouwbaar zijn**
   - Geen wijzigingen week voor event
   - Alles vooraf testen
   - Backup plannen klaar
   - Paper lijst als noodoplossing

2. **Uitbundige voorbereiding**
   - Minimum 2 proefritten (KRITISCH!)
   - Alle routes fysiek gereden
   - Challenges getest
   - Locaties bezocht

3. **Team conditie & gezondheid**
   - 17 uur dag = fysiek zwaar
   - Goede nachtrust voor event
   - Fit genoeg om hele dag te rijden?
   - Backup personen briefen

4. **Duidelijke communicatie**
   - Team weet exact wie wat doet
   - Noodprotocol doorgenomen
   - Deelnemers weten wat te verwachten
   - Roadbook is duidelijk

5. **Backup plannen overal**
   - Systeem faalt: Paper lijst
   - Teamlid ziek: Reserve persoon
   - Slecht weer: Alternatieve routes
   - Noodsituatie: Escalatie plan

### BELANGRIJK ⭐

1. **Proefrit = essentieel**
   - Met 2-3 teamleden
   - Hele route rijden
   - Challenges testen
   - Tijden checken
   - Gevaarlijke punten markeren

2. **Community mentality**
   - Team rijdt mee (niet boven deelnemers)
   - Deelnemers kunnen elkaar helpen
   - Social features promoten
   - Buddy systeem aanmoedigen

3. **Energie management**
   - Team pauzes inplannen
   - Niet non-stop focus
   - Geniet ook van de rit!
   - Lunch = reset moment

---

## 📊 FINANCIEEL OVERZICHT

### 🎯 Realistische Strategie: Maximum 50 Deelnemers (Eerste Event)

**Deelnemers Verwachting (Mix):**

- 35 deelnemers × €30 (All-in) = €1.050
- 15 deelnemers × €20 (Ontbijt only) = €300
- **Totaal registraties:** €1.350

### Inkomsten: Sponsoring is Cruciaal! 🤝

**Sponsoring Strategie:**

- **Ontbijt sponsoring** (bakkers, koffiemerk): €250-350
- **Goodiebags sponsoring** (volledige sponsoring): €150-200
- **Motormerken/winkels** (logo op materiaal): €300-500
- **Hospitality sponsors** (eindpunt, drankjes): €200-300
- **Lokale businesses**: €150-200
- **Totaal sponsoring target:** €1.050-1.550

**Totale inkomsten:** €2.400-2.900

### Uitgaven: Slimme Kostenbeheersing

**Eten & Drinken:**

- **Ontbijt** (50p, grotendeels gesponsord): €50-100 eigen bijdrage
- **Lunch** (35p All-in × €9 - croque + drankje): €315
- **Diner** (35p All-in - deal met eindpunt): €300-500
  - *Scenario A:* Deal met eindpunt (€300-350)
  - *Scenario B:* Geen deal, deftig arrangement (€450-500)
- **Subtotaal eten:** €665-915

**Materialen:**

- **Roadbooks** (50× × €6 printen): €300
- **Event verzekering**: €300
- **Spandoeken/signalisatie**: €0 (uitsparen eerste editie)
- **QR materialen**: €0 (via app geregeld)
- **Website hosting**: €0 (gratis)
- **Goodiebags**: €0 (volledig gesponsord)
- **Contingency** (onvoorzien): €150
- **Subtotaal materialen:** €750

**Totale uitgaven:** €1.415-1.665

### Budget Scenarios

**🟢 BEST CASE (met goede sponsoring + diner deal):**

- Inkomsten: €2.900 (registraties + sponsoring €1.550)
- Uitgaven: €1.415 (diner deal €300)
- **Saldo: +€1.485 overschot** ✅✅✅

**🟡 REALISTISCH (gemiddelde sponsoring + diner deal):**

- Inkomsten: €2.650 (registraties + sponsoring €1.300)
- Uitgaven: €1.515 (diner deal €350)
- **Saldo: +€1.135 overschot** ✅✅

**🟠 VOORZICHTIG (minimale sponsoring + duur diner):**

- Inkomsten: €2.400 (registraties + sponsoring €1.050)
- Uitgaven: €1.665 (diner €500)
- **Saldo: +€735 overschot** ✅

**🔴 WORST CASE (weinig sponsoring, geen diner deal):**

- Inkomsten: €2.100 (registraties + sponsoring €750)
- Uitgaven: €1.665
- **Saldo: +€435 overschot** ✅

### Conclusie: Budget Zeer Goed Sluitend! 🎉

**Zelfs in worst case scenario blijft er €435+ over!**

**Belangrijkste Succesfactoren:**

1. ✅ **Sponsoring ontbijt** (bakkers, koffiemerken) - cruciaal maar haalbaar
2. ✅ **Goodiebags sponsoring** - lokale motorzaken willen graag zichtbaarheid
3. ✅ **Diner deal met eindpunt** - win-win (zij krijgen klanten, wij korting)
4. ✅ **50 deelnemers is realistisch** voor eerste editie
5. ✅ **Kosten laag houden** door slimme keuzes (geen spandoeken, app voor QR)

**Financiële Strategie:**

- **Focus op sponsoring:** Ontbijt + goodiebags volledig gesponsord krijgen
- **Diner deal onderhandelen:** Met eindpunt afspraken maken
- **Lunch simpel houden:** Croque monsieur + drankje = betaalbaar & lekker
- **Eerste editie: Kwaliteit over kwantiteit** (50p is perfect voor testen)
- **Overschot = Buffer** voor volgende editie of onvoorziene kosten

**Target Sponsoring:**

| Type | Target | Prio | Status |
|------|--------|------|--------|
| Ontbijt (bakker/koffie) | €300 | 🔴 Hoog | Te zoeken |
| Goodiebags | €175 | 🔴 Hoog | Te zoeken |
| Motorwinkels | €400 | 🟡 Medium | Te zoeken |
| Hospitality/eindpunt | €250 | 🟡 Medium | Te onderhandelen |
| Lokale business | €175 | 🟢 Laag | Te zoeken |
| **Totaal** | **€1.300** | | |

**Bij €1.300 sponsoring + diner deal → €1.135 overschot!** 💰

---

## 🎯 FINALE CONCLUSIE

### Event Assessment: ⭐⭐⭐⭐⭐ (5/5)

**Concept:** Uniek, professioneel, community-driven
**Tech:** Zeer goed uitgewerkt, feature-rijk
**Team:** 5 personen is werkbaar met deze opzet
**Financiën:** ✅ Zeer goed! Zelfs in worst case €435+ overschot
**Risico's:** Beheersbaar met goede voorbereiding

### Team van 5 Personen: ✅ HAALBAAR

**Mét deze voorwaarden:**

1. ✅ Team rijdt mee (niet statisch wachten)
2. ✅ Tech 100% stabiel & getest
3. ✅ Minimum 2 proefritten gedaan
4. ✅ Backup plannen voor alles
5. ✅ 1 reserve persoon gebrieft (backup)
6. ✅ Team is fit & gezond op eventdag
7. ✅ Deelnemers zijn zelfredzaam (duidelijke info)

### Unique Selling Points

**Dit event is bijzonder omdat:**

- ✨ Team rijdt mee (authentiek, samen beleven)
- ✨ Geen competitie, wel gamification (leuk zonder stress)
- ✨ Systeem ondersteunt, maar mensen centraal
- ✨ Community building is het doel
- ✨ Professioneel maar toegankelijk
- ✨ Vrijheid binnen structuur (eigen routes kiezen)
- ✨ Herinneringen blijven leven na event

### Grootste Succesfactoren

1. **Proefritten** - Ken de route uit je hoofd
2. **Systeem betrouwbaarheid** - Test alles vooraf
3. **Team gezondheid** - 17u is een marathon
4. **Backup persoon** - Bij ziekte/nood
5. **Sponsoring strategie** - Ontbijt + goodiebags gesponsord krijgen
6. **Community spirit** - Deelnemers helpen elkaar

### Grootste Risico's

1. 🔴 Systeem storing tijdens event (HIGH)
2. 🟡 Teamlid ziek op eventdag (MEDIUM)
3. 🟡 Meerdere noodsituaties tegelijk (MEDIUM)
4. 🟢 Sponsoring niet rondkrijgen (LAAG - maar zelfs zonder blijft event winstgevend)
5. 🟢 Financieel (GEEN RISICO - €435-1.485 overschot afhankelijk van sponsoring)

---

## 🚀 GO / NO-GO CRITERIA

### ✅ GO als

- [ ] All 5 teamleden confirmed & committed
- [ ] Minimum 2 proefritten gedaan (met challenges getest)
- [ ] App/website volledig getest (alles werkt?)
- [ ] Kernsponsors bevestigd (ontbijt + goodiebags minimaal)
- [ ] Locaties geboekt & confirmed
- [ ] Verzekeringen geregeld
- [ ] Backup persoon gebrieft
- [ ] Noodprotocol doorgenomen met team
- [ ] Roadbook compleet & geprint
- [ ] 1 week voor event: Geen wijzigingen meer!

### 🛑 NO-GO als

- [ ] Minder dan 4 teamleden beschikbaar
- [ ] Geen proefrit gedaan
- [ ] App/website niet stabiel (crashes, errors)
- [ ] Geen enkele sponsor (zeer onwaarschijnlijk - budget blijft zelfs dan positief)
- [ ] Event verzekering niet OK
- [ ] Team niet fit/voorbereid
- [ ] Slecht weer zonder alternatief plan

---

**Dit event is financieel solide, technisch goed uitgewerkt, en haalbaar met 5 personen! 🏍️💪**

**Key Takeaways:**

- ✅ 50 deelnemers = perfect voor eerste editie
- ✅ Budget sluit zelfs zonder sponsoring (maar sponsoring maakt het véél beter)
- ✅ Systeem draait grotendeels vanzelf - jullie kunnen meerijden!
- ✅ Team rijdt mee = authentieke ervaring
- ✅ Risico's beheersbaar met goede voorbereiding

*Last updated: 6 februari 2026*
