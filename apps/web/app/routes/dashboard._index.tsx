          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-3 flex-wrap">
                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      if ('serviceWorker' in navigator) {
                        const registration = await navigator.serviceWorker.ready;
                        info.push(`✓ Service Worker: ${registration.active?.state || 'unknown'}`);
                        
                        // Check subscription
                        const subscription = await registration.pushManager.getSubscription();
                        if (subscription) {
                          info.push(`✓ Push Subscription: Active`);
                          info.push(`  Endpoint: ${subscription.endpoint.substring(0, 50)}...`);
                        } else {
                          info.push(`✗ Push Subscription: None`);
                        }
                      } else {
                      ...
                      // Check VAPID key
                      const vapidKey = (window.ENV as any)?.VITE_VAPID_PUBLIC_KEY;
                      if (vapidKey) {
                        info.push(`✓ VAPID Key: Configured (${vapidKey.substring(0, 20)}...)`);
                      } else {
                        info.push(`✗ VAPID Key: Missing`);
                      }
                      
                      const message = info.join('\n');
                      setPushDebugInfo(message);
                      alert('Push Notification Status:\n\n' + message);
                    } catch (err) {
                      alert(`Debug error: ${err instanceof Error ? err.message : 'Unknown'}`);
                    }
                  }}
                ...
                <button
                  onClick={async () => {
                    if (!confirm('Service worker opnieuw installeren? Dit kan helpen als push notifications niet werken.')) {
                      return;
                    }
                    ...
                    try {
                      ...
                      console.log(`[SW Reset] Found ${registrations.length} service worker(s)`);
                      
                      for (const registration of registrations) {
                        console.log('[SW Reset] Unregistering:', registration.scope);
                        await registration.unregister();
                      }

                      // Clear all caches
                      const cacheNames = await caches.keys();
                      console.log(`[SW Reset] Found ${cacheNames.length} cache(s)`);
                      ...
                      alert('✓ Service worker verwijderd!\n\nHerlaad de pagina om opnieuw te installeren.');
                      ...
                      console.error('[SW Reset] Error:', err);
                      alert(`Fout bij resetten: ${err instanceof Error ? err.message : 'Unknown'}`);
                    }
                  }}
